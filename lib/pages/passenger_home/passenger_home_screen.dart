...
class _PassengerHomeScreenState extends State<PassengerHomeScreen> with AutomaticKeepAliveClientMixin<PassengerHomeScreen>, WidgetsBindingObserver {
  ...
  void _startAutoRefreshTimer() {
    ...
    print('⏰ PassengerHomeScreen: Timer de atualização automática iniciado (30s)');
  }
  ...
  Future<void> _performAutoRefresh() async {
    ...
    try {
      ...
      print('✅ PassengerHomeScreen: Atualização automática concluída com sucesso');
    } catch (e) {
  ...
  void showPassagerDetailHome(BuildContext context, Ride ride) {
    ...
    if (ride.status.toUpperCase() != 'PENDING') {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Esta carona já não está mais disponível (status: ${ride.status})'),
  ...
  Future<void> _checkForAcceptedRequests() async {
    ...
    try {
      ...
      final requests = await RideService.getRideRequestsByPassenger(passengerIdInt);
      ...
      if (acceptedRequests.isNotEmpty && mounted) {
        ...
        for (final request in acceptedRequests) {
          ...
          if (rideId != null) {
            try {
              final int rideIdInt = rideId is int ? rideId : int.parse(rideId.toString());
              final rideDetails = await RideService.getRideById(rideIdInt);
              if (rideDetails != null) {
                final driverName = rideDetails['driver']?['name'] ?? 'Motorista';
                  ...
                  SnackBar(
                    ...
                    action: SnackBarAction(
                      ...
                      onPressed: () {
                        ...
                        print('Navegar para detalhes da viagem aceita: $rideId');
                      },
                    ),
                  ),
                );
              ...
              child: FutureBuilder<List<Ride>>(
                ...
                builder: (context, snapshot) {
                  ...
                  final availableRides = rides.where((ride) {
                    ...
                    if (!isAvailable) {
                      print('PassengerHomeScreen: Ocultando viagem ${ride.id} - status: $status');
                    }
                    return isAvailable;
                  }).toList();
...
class _CaronaCardState extends State<CaronaCard> {
                      ...
                      FutureBuilder<User>(
                        ...
                        builder: (context, userSnapshot) {
                          ...
                          if (userSnapshot.connectionState == ConnectionState.waiting) {
                            ...
                            displayName = '${widget.ride.vehicle.brand} ${widget.ride.vehicle.model}';
                          } else {
    ...
    return CustomButton(
      ...
      onPressed: () {
        ...
        if (widget.ride.status.toUpperCase() != 'PENDING') {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Esta carona já não está mais disponível (status: ${widget.ride.status})'),
              backgroundColor: Colors.orange,
            ),
          );
          return;
  ...
  Widget _buildInfoItem(BuildContext context, IconData icon, String label, String value, {bool isPrice = false}) {
    return Row(
      children: [
        Icon(
          icon,
