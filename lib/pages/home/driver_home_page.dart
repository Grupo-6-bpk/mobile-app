...
class _DriverHomePageState extends State<DriverHomePage> with WidgetsBindingObserver {
    ...
    try {
      ...
      if (_activeRide != null) {
        ...
        if (RideService.isValidRideId(rideId)) {
          print('🔄 DriverHomePage: Verificando solicitações pendentes para viagem $rideId');
    ...
    if (args != null) {
      try {
        ...
        if (args.containsKey('rideStarted') && args['rideStarted'] == true) {
          ...
          final startLocation = args['startLocation'] as String? ?? 'Não informado';
          ...
          if (acceptedPassengers.isNotEmpty) {
            ...
            for (int i = 0; i < acceptedPassengers.length; i++) {
              ...
              print('   📍 Início: ${passenger['startLocation'] ?? 'Não informado'}');
              print('   🎯 Fim: ${passenger['endLocation'] ?? 'Não informado'}');
  ...
  Future<void> _checkForActiveRide() async {
    ...
    if (currentUser?.userId == null) {
      print('DriverHomePage: Usuário não autenticado, pulando verificação de viagem ativa');
      setState(() => _isLoadingRide = false);
      return;
    }
    
    print('DriverHomePage: Verificando viagem ativa para usuário ${currentUser!.userId}');
    ...
    try {
      ...
      if (mounted) {
        ...
        if (!hadActiveRide && hasActiveRide) {
          print('✅ DriverHomePage: Nova viagem ativa encontrada: ${activeRide!['id']}');
      ...
      print('❌ DriverHomePage: Erro ao verificar viagem ativa: $e');
      if (mounted) {
        setState(() => _isLoadingRide = false);
      }
    }
  ...
  Widget _buildDriverHomePage() {
    ...
    final String startLocation = args?['startLocation'] ?? _activeRide?['startLocation'] ?? 'Não informado';
    final String endLocation = args?['endLocation'] ?? _activeRide?['endLocation'] ?? 'Não informado';
      ...
      child: Column(
        ...
        children: [
            ...
            child: Row(
              ...
              children: [
                Column(
                  ...
                  children: [
                    Text(
                      motorista != null ? 'Olá, ${motorista['name']}' : 'Boa tarde, Gabriel',
                      style: Theme.of(
                        context,
          ...
          if (corridaIniciada) ...[
            Padding(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text('Corrida em andamento', style: Theme.of(context).textTheme.titleMedium?.copyWith(fontWeight: FontWeight.bold)),
                  ...
                  if (passageiros.isNotEmpty) ...[
                    Text('Passageiros:', style: Theme.of(context).textTheme.titleSmall),
                  ...
                  CustomButton(
                    ...
                    onPressed: () async {
                      ...
                      final rideId = RideService.extractRideId(_activeRide) ?? RideService.extractRideId(args);
                      if (RideService.isValidRideId(rideId)) {
                        print('DriverHomePage: RideId para cancelamento: $rideId');
                        final int rideIdInt = rideId!;
                        print('DriverHomePage: RideId convertido para int: $rideIdInt');
                        
                        final success = await RideService.cancelRide(rideIdInt);
                        if (success && mounted) {
                          print('DriverHomePage: Viagem cancelada com sucesso');
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('Corrida cancelada com sucesso!'), backgroundColor: Colors.green),
                          );
                          // Resetar estado
                          setState(() {
                            _activeRide = null;
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(content: Text('Erro ao cancelar corrida'), backgroundColor: Colors.red),
                          );
                        }
                      } else {
                        print('DriverHomePage: ERRO - RideId não encontrado ou inválido para cancelamento');
                        print('DriverHomePage: _activeRide: $_activeRide');
                        print('DriverHomePage: args: $args');
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(content: Text('Erro: ID da viagem não encontrado ou inválido'), backgroundColor: Colors.red),
                        );
                      }
                    },
                  ),
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  ...(_acceptedPassengers.map((passenger) => _buildAcceptedPassengerCard(passenger))),
          ...
          Expanded(
            child: Padding(
              padding: const EdgeInsets.symmetric(horizontal: 20),
              child: _acceptedPassengers.isNotEmpty 
                  ? const Center(
                      child: Text(
                  ...
                  : ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        if ((passenger['user']?['name'] ?? passenger['name']) != null && (passenger['user']?['phone'] ?? passenger['phone']) != null) {
                          return AvailablePassengerCard(
                            name: passenger['user']?['name'] ?? passenger['name'] ?? 'Passageiro',
                            location: passenger['location'],
                            phoneNumber: passenger['user']?['phone'] ?? passenger['phone'] ?? 'Não informado',
  ...
  Widget _buildActionButton() {
    ...
    if (_activeRide != null) {
      ...
      final rideStatus = _activeRide!['status']?.toString()?.toUpperCase() ?? 'PENDING';
      final statusesQueIndicamPartida = ['IN_PROGRESS', 'COMPLETED', 'CANCELLED', 'FINISHED'];
      
      if (statusesQueIndicamPartida.contains(rideStatus)) {
        print('DriverHomePage: Viagem já partiu ou foi finalizada (status: $rideStatus), ocultando botão Gerenciar Viagem');
      ...
      return CustomButton(
        ...
        onPressed: () {
          ...
          if (!RideService.isValidRideId(extractedRideId)) {
            print('DriverHomePage: ERRO - RideId inválido para gerenciar viagem');
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Erro: ID da viagem inválido'), backgroundColor: Colors.red),
            );
            return;
          }
          ...
          print('DriverHomePage: Navegando para /ride_start com dados: $rideData');
          print('DriverHomePage: RideId confirmado: ${rideData['rideId']}');
          
          Navigator.pushNamed(context, '/ride_start', arguments: rideData)
              .then((result) {
                // Se a viagem foi cancelada, atualizar os dados
                if (result == 'cancelled') {
                  _checkForActiveRide();
                  setState(() {
                    _acceptedPassengers.clear();
